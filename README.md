...
## Current Update
2026-01-24: Lots of changes and the UI is doing about 95% of everything it did in the prototype stage -- but now fully against the database.  Removing the last of the hard coded tier levels in the code and nearly everything is dynamic.  Adding Intellivision now to properly test having a new platform along with Vic and C64.  Commodore places pixels in pairs, so adding the ability to place them individually with a single color for TI 99 and Intellivision.
