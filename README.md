...
## Current Update
2025-12-23: Finally completed hand edits on the whole database which is now loading correctly.  Developed a new load system to assist with this and throws out errant SQL lines from stderr.  Now going to create software to check integrity of the data.   It will check for dupes, and for proper drop down selections for comboboxes, and also will open the .glade file and ensure the widget exists in the GUI.  Thinking a bit now about proper ways to make DB updates on future updates.   Next up will be getting the actual screen to open again in the new format -- quite a big change as the flat files are retired.
